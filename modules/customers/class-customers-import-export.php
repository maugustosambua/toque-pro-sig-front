<?php // modules/customers/class-customers-import-export.php
...
class TPS_Customers_Import_Export {
    ...
    public static function import() {
        ...
        while ( ( $row = fgetcsv( $handle, 0, $delimiter ) ) !== false ) {
            ...
            $data = array(
                'type'    => trim( $row[ $indexes['type'] ] ?? '' ),
                'name'    => trim( $row[ $indexes['name'] ] ?? '' ),
                'nuit'    => trim( $row[ $indexes['nuit'] ] ?? '' ),
                'email'   => trim( $row[ $indexes['email'] ] ?? '' ),
                'phone'   => trim( $row[ $indexes['phone'] ] ?? '' ),
                'address' => trim( $row[ $indexes['address'] ] ?? '' ),
